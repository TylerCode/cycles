...
### Fixed
- Window content no longer renders cramped in the corner on first launch;
  `Resize` is now called after `Show` so the layout fills the window from
  the first frame.
- Reduced cognitive complexity in `DrawAreaChart`, `DrawRadialGauge`, and
  `parseMemInfo` by extracting helper functions (SonarCloud go:S3776).
